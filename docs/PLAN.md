...
**(n) `insertPullAhead` — formal amendment for Step 21 repo extension.** ✅ Shipped in Step 21.
`ItemMemoryStateRepository.insertPullAhead(state)` was added to realize the pull-ahead tap invariant in UI. It is a thin wrapper: delegates to `this.insert(this.db, state)`, the same SQL path that `ReviewEventRepository.recordFirstReview()` calls inside its transaction — no duplicate insert logic. The `ItemMemoryState` passed to it is constructed in `this-week.tsx` using `SchedulerService.createInitialState(itemId, now)`, not hand-built FSRS fields (the difficulty-0 lesson applies: no raw `{stability:0, difficulty:0, ...}` literals). Consequence: pull-ahead introduces an item with a `due = now` timestamp and `state='New'`, so it arrives via `dueStates` on the next queue build as a `kind='review'` entry and bypasses `newItemCap` — consistent with the QueueBuilder contract.

**(o) Day-boundary change does not recompute stored study-day strings — known MVP limitation.** Open.
`dayBoundaryHour` in Settings (0–6 am range) changes how future `getStudyDay()` calls resolve, but stored strings (`lastQualifyingDate` in `ItemMemoryState`, `lastDrillDate` in the drill streak) were written under the old boundary and are not retroactively updated. A student who tightens the boundary (e.g., 4am → 2am) could allow a relearn double-advance within the same calendar day, or a streak continuation that should not have fired, in the session immediately after the change. This is acceptable at MVP because: (1) the setting is labeled "advanced"; (2) valid use cases (shifting from night-owl to morning study pattern) mean the impact is usually benign; (3) the Settings UI now shows a one-line confirmation note. Future mitigation if needed: rewrite stored dates on boundary change using a migration function. Until then, do NOT implement mid-session boundary changes (the setting only takes effect on the next app launch or queue build).
